    ...
    <div className="min-h-screen bg-[#10151c] flex flex-col items-center py-12 px-2">
      ...
      {!authed ? (
        <form onSubmit={handleLogin} className="bg-white rounded-2xl shadow-xl p-8 flex flex-col gap-4 w-full max-w-xs">
          ...
          <input type="text" value={username} onChange={e => setUsername(e.target.value)} className="rounded-lg border border-gray-200 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#ADFF00] bg-[#f8fafc]" required />
          <label className="font-semibold text-[#1F2E47]">Password</label>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="rounded-lg border border-gray-200 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#ADFF00] bg-[#f8fafc]" required />
