  ...
  const JobCard = ({ application }: { application: any }) => {
    ...
    async function handleSubmitSubmission(e: React.FormEvent) {
      ...
      try {
        ...
        // Show visual confirmation inside the modal
        ...
        // Auto-close and reset the form shortly after showing success
        setTimeout(() => {
                  ...
                  {submitSuccess ? (
                    <div className="flex flex-col items-center text-center py-4">
                      ...
                      <p className="text-xs text-gray-600">
                        We have received your file. You can track the status in
                        Completed.
                      </p>
                      <Button
                        type="button"
                        className="mt-4 bg-skill hover:bg-purple-700 text-white px-4"
                        onClick={() => setShowSubmitModal(false)}
                      >
                        Done
                      </Button>
                    ...
                    <form onSubmit={handleSubmitSubmission}>
                      <div className="mb-4">
                        ...
                        <div className="relative border-2 border-gray-300 border-dashed rounded flex flex-col items-center justify-center py-7 cursor-pointer hover:border-purple-400 transition-all">
                          ...
                          <span className="text-gray-500 text-sm">
                            {submissionFile ? "Replace File" : "Upload File"}
                          </span>
                        </div>

                        {/* Confirmation chip + thumbnail */}
                        {submissionFile && (
                          <div className="mt-3 flex items-center gap-3">
                            ...
                            {submissionFile.type.startsWith("image/") ? (
                              ...
                              <div className="w-10 h-10 rounded bg-gray-100 grid place-items-center border text-xs">
                                PDF
                              </div>
                            )}

                            {/* Chip with name + size */}
                            <div className="flex items-center gap-2 bg-green-50 text-green-700 border border-green-200 px-3 py-1 rounded-full text-xs">
                              ...
                              <button
                                type="button"
                                className="ml-1 text-green-800/70 hover:text-green-900"
                      ...
                      {/* Optional indeterminate bar during upload */}
                      {isSubmitting && (
                        <div className="w-full h-1 bg-gray-200 rounded overflow-hidden mb-3">
                          <div className="h-full w-1/3 bg-purple-500 animate-[progress_1.2s_linear_infinite]" />
                          <style jsx>{`
                            @keyframes progress {
                              0% {
            ...
            <div className="flex gap-4">
              {getFilterTabs().map((tab, index) => (
                <Button
                  ...
                  className={`${
                    tab.active
                      ? "bg-white text-black hover:bg-gray-800 border-gray-600 hover:text-white"
