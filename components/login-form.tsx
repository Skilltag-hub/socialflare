...
export default function LoginForm() {
  ...
  // Tabs removed: no dynamic height animation needed
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res && res.ok) {
      ...
      try {
        ...
        router.push("/login/details"); // Default to details page on error
      }
    } else {
            ...
            >
              {loading ? (
                <div className="flex items-center justify-center space-x-2">
                  <div className="h-5 w-5 rounded-full border-t-2 border-b-2 border-[#4285F4] animate-spin"></div>
