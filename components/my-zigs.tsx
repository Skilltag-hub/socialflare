...
export default function MyZigs() {
  ...
  async function fetchJobs() {
    try {
      ...
      const formattedJobs = gigs.map((job, index) => {
        const formattedJob = {
              ...
              <div key={job._id} className="bg-white text-black rounded-xl shadow-sm h-[220px] flex flex-col hover:shadow-lg transition-shadow cursor-pointer" onClick={() => window.location.href = `/companies/job-applications/${job._id}`}>
                <div className="p-3 flex flex-col h-full">
                  <div className="mb-3 flex-1">
                    <h3 className="font-normal text-gray-700 leading-relaxed text-sm line-clamp-1">{job.gigTitle}</h3>
                    <p className="text-xs text-gray-500 line-clamp-2">{job.description}</p>
                    {job.payment && (
                      <p className="text-xs font-medium text-gray-700 mt-1">Payment: {job.payment}</p>
                    )}
